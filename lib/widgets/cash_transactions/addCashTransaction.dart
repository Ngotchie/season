...
class AddCashTransaction extends StatefulWidget {
  const AddCashTransaction({Key? key}) : super(key: key);

  @override
  _AddCashTransactionState createState() => _AddCashTransactionState();
}
...
class _AddCashTransactionState extends State<AddCashTransaction> {
  ...
  final accountingPlanController = TextEditingController();
  final amountController = TextEditingController();
  final currencyController = TextEditingController();
  final typeTransactionController = TextEditingController();
  ...
  Widget build(BuildContext context) {
    ...
    var cashBoxes = [];
    var accountingPlans = [];
    // FormAccommodation accInit =
    //     new FormAccommodation(0, "Choose", "accommodation...");
    // accommodations.add(accInit);
    return Scaffold(
        appBar: AppBar(
            ...
            builder: (context, AsyncSnapshot snapshot) {
              if (snapshot.data != null) {
                ...
                final dateFormField = DateTimeField(
                  ...
                  validator: (value) {
                    if (value == null) {
                      return 'Transaction date can\'t be empty';
                    }
                    return null;
                  },
                );
                          ...
                          new ElevatedButton(
                              ...
                              onPressed: () {
                                if (_formkey.currentState!.validate()) {
                                  ...
                                  this.response["created_by_user_id"] = user.id;
  ...
  _postData(formData) async {
    var resp = await apiCash.postCashTransaction(formData);
    if (resp == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Cash transaction adding succefully')),
      );
      Navigator.pushNamed(context, '/cashTransaction');
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error during add cash transaction')),
      );
    }
  }
}
